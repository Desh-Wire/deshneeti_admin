    ...
    <>
      <div className="min-h-screen bg-[#ece2c8] p-6">
        <main className="bg-white rounded-lg shadow-lg p-6 max-w-6xl mx-auto">
          ...
          <div className="mt-8">
            {error ? (
              <p className="text-red-500 text-center">Failed to load categories: {error.message}</p>
            ) : (
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-8">
                  {data?.categories.map((category: any) => (
                    <div key={category.id} className="p-6 bg-gray-100 rounded-lg shadow-md text-center hover:shadow-lg transition-shadow duration-300">
